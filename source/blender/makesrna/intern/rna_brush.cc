...
static void rna_def_brush(BlenderRNA *brna)
{
  ...
  prop = RNA_def_property(srna, "use_project_local", PROP_BOOLEAN, PROP_NONE);
  RNA_def_property_boolean_sdna(prop, nullptr, "flag2", BRUSH_PROJECT_LOCAL);
  RNA_def_property_ui_text(prop,
                           "Project Local",
                           "Brush is a project-local customization saved in the .blended file. "
                           "Brushes without this flag are transient defaults regenerated by "
                           "paint-mode initialization and are not persisted across reloads");
  RNA_def_property_update(prop, 0, "rna_Brush_update");
