...
**Launcher aesthetic (settled 2026-05-16):** Hybrid of Adobe Creative Cloud home screen and Blender splash screen. Adobe contributes dark card surfaces, left sidebar primary actions, thumbnail file cards (AssetCard pattern), and typographic restraint. Blender contributes the New File / Recent Files startup logic. §11 pipeline scroll is Blended's own. Three-level dark surface: `#1D1D1D` base → `#252525` panel → `#2C2C2C` card (resting). Interaction states separate: hover `#323232`, active/pressed = accent (Phase 2). Left sidebar ~220px: wordmark, [New Project] CTA, [Open…], recent `.blended` file thumbnail cards (~160×120px, 8px corners), project settings gear, version + CHJ 3 attribution. Pipeline scroll in main area: "Blending?" 28px SemiBold, group separators 11px all-caps letter-spaced (`#666666`), section headings 16px SemiBold, mode buttons as rounded cards (8px radius, hover lifts background). Type stack: Source Sans Pro / system sans-serif cascade → replaced by brand typeface in Phase 2. 8px spacing base unit. 130ms ease-out hover animation. Full spec in BLENDED.md §11.
