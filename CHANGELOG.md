...
**0.7.0 permanent home (2026-05-17):** Commit `9c8dbc3c`. `NodeCompositeMask` storage added to `DNA_node_types.h` with node-owned `Mask*`. `BKE_mask.hh` + `mask.cc`: `new_nodetree` / `copy_nodetree` / `free_nodetree` / `write_layers` / `read_layers` / `drain_from_bmain` helpers. `node_composite_mask.cc` rewritten with storage type + blend callbacks; `template_id` removed. Versioning 502.27: compositor CMP_NODE_MASK nodes migrate mask from `node.id` into `NodeCompositeMask` storage; sequencer strip and modifier mask references nullified (Category A). Drain called post-load in `readfile.cc`. Subversion 26 → 27. `bmain->masks` Category C leak resolved.
