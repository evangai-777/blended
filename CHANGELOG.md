...
**Claude AI contributor (2026-06-01 — 0.9.0 implementation):** All five layers implemented in a single commit on branch `claude/0-9-0-regression-LU6mH`, commit `fe95e326`. Layer 1: versioning_520.cc — added subversion 502.31 block converting OB_MBALL (value 5) to OB_EMPTY, populating `fd->reports->count.mball_converted` for Layer 5; bumped `BLENDER_FILE_SUBVERSION` to 31; updated `blo_do_versions_520` signature to use `fd` parameter. Layer 2: main.cc — after the indexed ID loop in `BKE_main_clear`, added lambda-based drain for all four non-indexed Scar 2 listbases (particles, textures, curves, linestyles) using explicit next-pointer iteration matching the existing indexed loop pattern. `ListBaseT<T>` inherits from `ListBase` — lambda takes `ListBase &` via upcast. Layer 3: readfile.cc — post-read drain for `bmain->linestyles` in `after_liblink_merged_bmain_process`, using explicit for-loop (LISTBASE_FOREACH_MUTABLE does not exist in this codebase — CLAUDE.md template had a wrong macro name; corrected to explicit loop). Layer 4: screen.cc + BKE_screen.hh — deleted `BKE_screen_blend_read_data` function body and header declaration; confirmed zero callers with full-repo grep. Layer 5: readfile.cc + BLO_readfile.hh — added `mball_converted` counter to `BlendFileReadReport::count`; in `after_liblink_merged_bmain_process` before the LS drain, count all Scar 2 listbases and OB_MBALL conversions; if any > 0, create a `Text` ID via `BKE_text_add` + `BKE_text_write`. Added `BKE_text.h` include to readfile.cc.
...
- 0.8.0 implementation session (2026-05-31): all ~80 blast radius sites implemented across 10 commits on branch `claude/0.8.0-blast-radius-audit-Sa2tB`. Layers 1-23 complete. Key implementation decisions: `file_is_blend_backup()` rewritten as two-pass detector (`.blend` at 7-8 chars, `.blended` at 9-10 chars); autosave extension strip switched from hardcoded `-6` to `BLI_path_extension()` dynamic; `FILE_TYPE_BLENDED = (1 << 20)` placed after `FILE_TYPE_VOLUME = (1 << 19)`; `.blended` checked before `.blend` everywhere substrings could collide; macOS UTI `com.chj3productions.blended.file` added to Info.plist; Linux `application/x-blended` MIME + `*.blended` glob added to freedesktop.py; Windows `.blended` registry key added alongside `.blend`. Scar 20 verification grep clean — all stale `.blend` hits in source are either intentional read-compat references or docs.
- Two Codex catches fixed (PR 225, commit `8da6ed43`): (1) `filter_blender` / `filter_blended` isolation — collapsing both into `filter_blender` caused save-only dialogs (`FILE_TYPE_BLENDED`) to re-expand via `filesel.cc` to include `FILE_TYPE_BLENDER`, showing `.blend` files in native-format-only save dialogs; fixed by splitting into two separate operator properties (`filter_blender` → `FILE_TYPE_BLENDER` only, `filter_blended` → `FILE_TYPE_BLENDED` only) in both `wm_operator_props.cc` and `filesel.cc`. (2) `.blend` registry `lresult` clobber in `winstuff.cc` — a `.blend` registration failure was silently overwritten by a successful `.blended` result, returning true while `.blend` import-compat association was not registered; fixed by checking `lresult` immediately after the `.blend` block.
...
**Launcher aesthetic (settled 2026-05-16, chrome packaging clarified 2026-05-16):** Hybrid of Adobe Creative Cloud home screen and Blender splash. The pipeline scroll is the locked core — "Blending?" + §11 Creative/Post sections + mode button cards, centered, full screen. File management chrome (wordmark, [New Project], [Open…], recent file thumbnail cards ~160×120px, project settings, version + CHJ 3 attribution) is implementation-flexible: fixed left sidebar (~220px) OR compact top dropdown; content identical either way. Three-level dark surface: `#1D1D1D` base → `#252525` panel → `#2C2C2C` card (resting). Interaction states: hover `#323232`, active/pressed = accent (Phase 2). Mode button cards: 8px radius, 16px/12px padding, 130ms ease-out hover. Type stack: Inter (Blender's bundled interface font) — permanent. 8px spacing base unit. Full spec in BLENDED.md §11.
...
- [x] VFont → filepath on OB_FONT; drain `bmain->fonts` (all layers: DNA filepath fields on `Curve` struct + versioning pass 502.24 + RNA sync callback + BKE_curve_vfont_ensure + post-read drain)
- [x] Palette → inline into `Brush` struct; drain `bmain->palettes` (all layers: DNA embed PaletteColor/Palette in Brush + Paint::palette deprecated, brush.cc copy/free/I/O, paint.cc API, editors updated, versioning pass 502.25 + drain)
- [x] LightProbe → `eLightType` expansion + field migration + versioning pass; drain `bmain->lightprobes` (all layers: eLightType LA_PROBE_SPHERE=5/LA_PROBE_PLANAR=6/LA_PROBE_VOLUME=7 + ~30 probe_* fields in Light DNA, object_add.cc OB_LAMP, EEVEE/overlay cast-to-Light, versioning pass 502.26, BKE_lightprobe_drain_from_bmain, commit `a336e0b2`)
- [x] Mask → embed in compositor `NodeTree`; drain `bmain->masks` (all layers: NodeCompositeMask storage + node init/free/copy/blend callbacks, BKE_mask nodetree helpers + drain, versioning pass 502.27, commit `9c8dbc3c`)
- [x] Lattice → embed in `LatticeModifierData`; drain `bmain->lattices` (all layers: LatticeModifierData DNA + object_to_lattice[4][4], BKE lifecycle helpers + inline deform funcs, MOD_lattice refactor, RNA/editor cleanup, versioning pass 502.29 + OB_LATTICE→OB_EMPTY + GP lattice null, BKE_lattice_drain_from_bmain, subversion 29, commits `95ff32e2`–`9166a297`)
- [x] Brush → project-optional annotation; drain `bmain->brushes` when not needed (`BRUSH_PROJECT_LOCAL` flag in `eBrushFlags2`, versioning pass 502.30 flags all legacy brushes, `BKE_brush_drain_transient` in brush.cc + readfile.cc hook, `use_project_local` RNA prop, subversion 30, commits `0095ce44`–`0f47294c`)
...
**Claude AI contributor (2026-05-16, continued):** 0.7.0 Phase 1 — VFont layers 3–5 + Palette all layers. Branch `claude/continue-0.7.0-zvzZR`. 8 commits. Key work: (1) **VFont Layer 3** — RNA sync callback in `rna_curve.cc`: `prop_vfont_update` syncs `font_filepath` from `vfont->filepath` whenever the vfont* pointer is updated in the UI, keeping filepath fields authoritative. (2) **VFont Layer 4** — `BKE_curve_vfont_ensure(Curve *cu)` added to `vfont.cc`/`BKE_vfont.hh`: pre-populates all four vfont* pointers from filepath at the entry of each `BKE_vfont_to_curve*` call so the runtime layout pipeline works after `bmain->fonts` is drained. (3) **VFont Layer 5** — `BKE_vfont_drain_from_bmain(bmain)` added; clears vfont* pointers on all `Curve` objects then frees every VFont block from the Scar 2 `bmain->fonts` listbase; called from `after_liblink_merged_bmain_process` in `readfile.cc`. VFont Bucket 3 all layers complete. (4) **Palette Layer A** (DNA) — Moved `PaletteColor`/`Palette` struct definitions before `Brush` in `DNA_brush_types.h`; added `Palette palette = {};` as embedded field on `Brush`; marked `Paint::palette` as `DNA_DEPRECATED`. (5) **Palette Layer B** (paint.cc) — `BKE_paint_palette()` now returns `&brush->palette` when an active brush exists (falls back to deprecated `paint->palette` for legacy load); `BKE_paint_palette_set()` no-op. (6) **Palette Layer C** (brush.cc) — `brush_copy_data` duplicates `palette.colors` list; `brush_free_data` frees it; `brush_blend_write` writes the list; `brush_blend_read_data` reads it. (7) **Palette Layer D** (scene.cc) — removed `paint->palette` `BKE_LIB_FOREACHID_UNDO_PRESERVE_PROCESS_IDSUPER_P` walker. (8) **Palette Layer E** (editors) — `palette.cc`: `palette_poll` checks brush editability instead of `paint->palette != nullptr`; all exec functions use `BKE_paint_palette(paint)`. `eyedropper_grease_pencil_color.cc`: removed "create palette if nullptr" block (embedded palette always present on active brush). (9) **Palette Layer F** (versioning + drain) — `BKE_palette_drain_from_bmain` in `paint.cc`/`BKE_paint.hh`; called from `after_liblink_merged_bmain_process`; versioning pass 502.25 clears deprecated `paint->palette` on all ToolSettings paint modes in every Scene; `BLENDER_FILE_SUBVERSION` bumped to 25. Palette → Brush all layers complete. `bmain->palettes` Category C leak resolved.

**Claude AI contributor (2026-05-17, continued — PR #191 Codex review fixes):** Mask permanent home follow-up fixes. Branch `claude/continue-0.7.0-zvzZR`. Commits `6324b269`, `4375e6f4`, `305a1e81`, `0d375e53`. (1) **Include hygiene** — `MEM_guardedalloc.h` missing from `node_composite_mask.cc` and `versioning_520.cc`; both call `MEM_new` without the explicit include; transitive chains are inconsistent across files of similar shape and must be grepped, not assumed. Added explicit includes. Documented as squirrel.md in CLAUDE.md wtf.md section; include hygiene check added to Codex pre-commit checklist. (2) **Codex Bug 1 — ID prefix** — `BKE_mask_new_nodetree` left `mask->id.name[0..1]` as `\0\0`; compositor cache keys on `std::string(mask->id.name)`, so node-owned masks with blank prefix aliased to the same cache entry when render params matched. Fix: set `*reinterpret_cast<short *>(mask->id.name) = ID_MSK` before STRNCPY_UTF8. (3) **Codex Bug 2 — metadata serialization** — `NodeCompositeMask` had only `Mask *mask` (runtime pointer); blend callbacks wrote/read layer heap data inline but top-level metadata (sfra, efra, flag, masklay_act, name) was never stored. On save/reload the mask was reconstructed with hardcoded defaults ("Mask", sfra=1, efra=100), losing any user-set frame range or migrated mask name. Fix (Option A): add `int sfra=1`, `int efra=100`, `int flag=0`, `int masklay_act=0`, `char name[66]={}`, `char _pad[6]={}` to `NodeCompositeMask` so SDNA handles serialization; `node_blend_write` syncs runtime mask fields into struct before writing layers; `node_blend_read` restores from struct into mask after reading layers (efra==0 sentinel for pre-502.28 files that lack the fields); versioning 502.27 block also populates struct metadata from migrated src_mask. Added `BLI_string.h` include for STRNCPY template (not in transitive chain). Subversion bumped 27 → 28.

**Claude AI contributor (2026-05-17, continued):** 0.7.0 Phase 1 — Mask permanent home all layers. Branch `claude/continue-0.7.0-zvzZR`. Commit `9c8dbc3c`. 7 files, 275 insertions, 8 deletions. Key work: (1) **DNA** — `NodeCompositeMask { Mask *mask; }` struct added to `DNA_node_types.h`; Mask pointer is node-owned, not a shared ID block reference. (2) **BKE** — `BKE_mask_new_nodetree` / `BKE_mask_copy_nodetree` / `BKE_mask_free_nodetree` helpers for node-owned Mask lifetime management (no ID system, no bmain listbase). `BKE_mask_write_layers` / `BKE_mask_read_layers` serialize mask layer → spline → point → UW → shape data inline via node blend callbacks. `BKE_mask_drain_from_bmain` sweeps `bmain->masks` Scar 2 listbase post-load. (3) **node_composite_mask.cc** — fully rewritten: `init` creates node-owned Mask, `free`/`copy` use nodetree helpers, `blend_write_storage_content` / `blend_data_read_storage_content` wire to write/read layers; `template_id` removed from declare (bpy.data.masks gone). (4) **Versioning 502.27** — `CMP_NODE_MASK` nodes with live `node.id` get their Mask deep-copied into `NodeCompositeMask` storage; `node.id` nullified + refcount decremented. Sequencer: `strip->mask` and all `StripModifierData::mask_id` set to nullptr (Category A: silently dropped). (5) **readfile.cc** — `BKE_mask_drain_from_bmain` called post-load. Subversion bumped 26 → 27. `bmain->masks` Category C leak resolved.

**Claude AI contributor (2026-05-17, continued — Lattice Codex bug-fix):** Two functional bugs caught by Codex review on the Lattice permanent home PR. Branch `claude/new-session-aYkA3`. Commit `97af74a4`. (1) **Bug 1 — drain use-after-free.** `BKE_lattice_drain_from_bmain` ran unconditionally in `after_liblink_merged_bmain_process`. Files at subversion ≥502.29 (new files with runtime OB_LATTICE objects) had `ob->data` pointing into `bmain->lattices`; unconditional drain freed those blocks mid-session, leaving dangling pointers. Fix: wrapped drain in `if (!MAIN_VERSION_FILE_ATLEAST(bmain, 502, 29))` — drains only legacy files that needed Lattice ID cleanup; post-502.29 Lattice blocks remain in `bmain->lattices` as a bounded Category C leak (deferred until OB_LATTICE is fully retired at 0.9.x). (2) **Bug 2 — lmd->object deform path dropped.** The 0.7.0 refactor wired `deform_verts` and `deform_verts_EM` to call the inline embedded-lattice path unconditionally, ignoring `lmd->object`. Lattice parenting (`Ctrl+P → Lattice Deform`) and add-to-selected workflows set `lmd->object` (never `lmd->lattice`), so deformation silently used the default 2×2×2 embedded lattice instead. Fix: restored `lmd->object` check as primary deformation source in both functions; `lmd->lattice` path used only when `lmd->object == nullptr`. `is_disabled` updated to return true only when both are null. Files: `readfile.cc`, `MOD_lattice.cc`.

**Claude AI contributor (2026-05-17, continued):** 0.7.0 Phase 1 — Lattice permanent home all layers. Branch `claude/start-0.7.0-lattice-T1svM`. 6 commits `95ff32e2`–`9166a297`. Key work: (1) **Layer A (DNA)** — `LatticeModifierData` gains `Lattice *lattice` (embedded geometry, non-ID owned) + `float object_to_lattice[4][4]` (transforms modified-object local → lattice local space, replaces runtime `inv(oblatt->world) * ob->world`); `Object *object` kept as deprecated field for lib-link/versioning bridge. `struct Lattice;` forward declaration added. (2) **Layer B (BKE lifecycle)** — 6 new functions in `lattice.cc` / `BKE_lattice.hh`: `BKE_lattice_new_modifier` (MEM_new + ID type byte + init_data), `BKE_lattice_copy_modifier` (MEM_dupalloc + deep-copy def/dvert/vertex_groups; excludes key/adt/editlatt), `BKE_lattice_free_modifier` (batch_cache + vertex_groups + def + dvert; no BKE_libblock_free_data — embedded non-ID allocation), `BKE_lattice_write_modifier` / `BKE_lattice_read_modifier` (inline blend I/O for embedded storage), `BKE_lattice_drain_from_bmain` (drains Scar 2 bmain->lattices via BKE_libblock_free_data + MEM_delete). (3) **Layer C (inline deform)** — `BKE_lattice_deform_data_create_inline` added to `lattice_deform.cc`: creates LatticeDeformData from embedded `Lattice *lt` + `object_to_lattice[4][4]` directly, no `Object *oblatt` needed. (4) **Layer D (modifier + deform coords)** — Two inline coord functions `BKE_lattice_deform_coords_with_mesh_inline` and `BKE_lattice_deform_coords_with_editmesh_inline` mirror the `_with_mesh` / `_with_editmesh` API but call `BKE_lattice_deform_data_create_inline`; `MOD_lattice.cc` fully rewritten: `init_data` = unit_m4 + BKE_lattice_new_modifier, `copy_data` deep-copies embedded lattice, `free_data` frees it (new, was nullptr), `deform_verts` / `deform_verts_EM` use inline deform functions, `blend_write` / `blend_read` serialize embedded Lattice inline, `update_depsgraph` removed, `panel_draw` removes "object" prop; `modifier.cc` VirtualModifierData dangling-pointer fix: null `virtualModifierCommonData.lmd.lattice` after the temporary modifier is freed. (5) **Layer E (RNA/editor cleanup)** — `rna_modifier.cc`: "object" RNA property removed from `rna_def_modifier_lattice`; `rna_Lattice_object_poll` kept (still used by GreasePencilLatticeModifier); `tree_element_modifier.cc`: OB_LATTICE outliner branch removed (lmd->object null post-502.29, null ID crashes add_element); `modifier.cc` `BKE_modifiers_is_deformed_by_lattice`: standard lattice path returns nullptr (lmd->object always null post-502.29); `particle.cc`: lattice deform prep uses `BKE_lattice_deform_data_create_inline` for embedded lattice with fallback to legacy `lmd->object` for pre-502.29 files. (6) **Layer F (versioning + drain)** — `BLENDER_FILE_SUBVERSION` 28 → 29; versioning 502.29 block: Step 1 deep-copies Lattice from OB_LATTICE objects into `lmd->lattice`, sets `object_to_lattice = identity` (Category A: deformation shifts for non-identity OB_LATTICE transforms), nulls `lmd->object`; Step 2 nulls GreasePencilLatticeModifier `lmd->object` refs (Category A: GP lattice deform silently drops for legacy GP lattice modifiers); Step 3 converts OB_LATTICE → OB_EMPTY to prevent dangling `ob->data` after drain; `BKE_lattice_drain_from_bmain` call added to `readfile.cc` post-load. `bmain->lattices` Category C leak resolved. Lattice permanent home all layers complete.

**Claude AI contributor (2026-05-17, continued):** 0.7.0 Phase 1 — Brush permanent home all layers. Branch `claude/new-session-aYkA3`. 4 commits `0095ce44`–`0f47294c`. Key work: (1) **Layer A (DNA)** — `BRUSH_PROJECT_LOCAL = (1 << 12)` added to `eBrushFlags2` enum in `DNA_brush_enums.h` (first free bit after bits 0–11; `BRUSH_PROJECT_USE_BIDIRECTIONAL` occupies bit 11). Flag marks project-local user customizations that should be serialized in the `.blended` file. (2) **Layer B (versioning + subversion)** — `BLENDER_FILE_SUBVERSION` bumped 29 → 30; versioning pass 502.30 in `versioning_520.cc` sets `BRUSH_PROJECT_LOCAL` on ALL brushes in files older than 502.30 — no legacy file loses brush data on first load. (3) **Layer C (drain + readfile hook)** — `BKE_brush_drain_transient(Main *bmain)` added to `brush.cc` and declared in `BKE_brush.hh`. Iterates `bmain->brushes` and frees every brush WITHOUT `BRUSH_PROJECT_LOCAL` using `brush_free_data` (static: curvemappings + gpencil settings + gradient + preview image) + `BKE_libblock_free_data` + `MEM_delete`. Drain lives in `brush.cc` to access `brush_free_data` without making it non-static. Cannot use the generic `BKE_id_free` template — that skips `brush_free_data` post-`INIT_TYPE` removal (ID_BR was deregistered in 0.5.0 fold-down), leaking curvemapping heap data. `BKE_brush_drain_transient` called from `after_liblink_merged_bmain_process` in `readfile.cc`; `#include "BKE_brush.hh"` added alongside other drain includes. Drain runs unconditionally (no version gate) — transient brushes are always cleared post-read; no Category C leak from `bmain->brushes`. (4) **Layer D (RNA)** — `use_project_local` boolean property added to `rna_brush.cc` (flag2 bit `BRUSH_PROJECT_LOCAL`); `RNA_def_property_ui_text` explains project-local vs transient semantics; update callback `rna_Brush_update`. All four mandatory docs updated. Brush permanent home complete. Phase 1 skeleton complete — all 6 Bucket 3 permanent homes done.

**Claude AI contributor (2026-05-16, continued):** 0.7.0 Phase 1 — LightProbe permanent home all layers. Branch `claude/continue-0.7.0-zvzZR`. Commit `a336e0b2`. 14 files, 283 insertions, 90 deletions. Key work: (1) **DNA** — `eLightType` extended with `LA_PROBE_SPHERE=5`, `LA_PROBE_PLANAR=6`, `LA_PROBE_VOLUME=7`; ~30 `probe_*` fields (`probe_resolution`, `probe_clipping_distance`, `probe_attenuation_distance`, etc.) migrated from LightProbe struct into `Light` struct in `DNA_lamp_types.h`. (2) **BKE** — `BKE_lightprobe_type_apply_to_light(LightProbe *probe, Light *la)` added for versioning; `BKE_lightprobe_drain_from_bmain(bmain)` added; called from `after_liblink_merged_bmain_process` in `readfile.cc`. (3) **object_add.cc** — Add > Light Probe menu items now create `OB_LAMP` objects with the appropriate `eLightType` instead of `OB_LIGHTPROBE`. (4) **EEVEE + overlay** — probe evaluation and draw code updated to cast `Object::data` to `Light *` and read `probe_*` fields directly. (5) **Versioning pass 502.26** — converts all `OB_LIGHTPROBE` objects to `OB_LAMP` with matching `eLightType`; populates Light `probe_*` fields from the original `LightProbe` block via `BKE_lightprobe_type_apply_to_light`; `BLENDER_FILE_SUBVERSION` bumped 25 → 26. LightProbe Bucket 3 all layers complete. `bmain->lightprobes` Category C leak resolved.

**Claude AI contributor (2026-05-17, continued):** 0.7.0 Phase 2 — Launcher aesthetic: file management chrome + rounded cards + hover state. Branch `claude/phase-2-ai-implementation-nd4ZZ`. Commits `a633d329` (implementation) + `cc37157f` (docs). PR #196. Key work: (1) **Launcher header chrome** — `RGN_TYPE_HEADER` region added to `ED_spacetype_blended_launcher()`; `launcher_header_region_init` calls `ED_region_header_init(region)`; `launcher_header_region_draw` calls `ED_region_header(C, region)` — identical pattern to `space_info`. `launcher_create()` now allocates both header and window regions; header alignment reads `U.uiflag & USER_HEADER_BOTTOM` (requires explicit `#include "DNA_userdef_types.h"`). `scripts/startup/blended_launcher.py` (new file): `LAUNCHER_HT_header(Header)` with `bl_space_type = 'BLENDED_LAUNCHER'`; draws wordmark label, separator, `wm.read_homefile` ("New Project"), `wm.open_mainfile` ("Open…"), `TOPBAR_MT_file_open_recent` menu ("Open Recent"). Auto-registered by Blender's `scripts/startup/` mechanism. (2) **Rounded mode button cards** — `draw_rect_rounded()` GPU triangle-fan helper: 4 corners × 6 segments each, 8px radius clamped to `min(w,h)/2`, `GPU_PRIM_TRI_FAN` with center vertex. `draw_rect_rounded_border()` draws 2px accent outline via four `draw_rect_filled` strips. (3) **Hover state** — `SpaceBlendedLauncher` DNA: `char _pad1[4]` replaced by `int mouse_x = -1; int mouse_y = -1;` (same struct size, two int fields). `launcher_main_region_cursor()` cursor callback stores mouse position into `sl->mouse_x/y` and calls `ED_region_tag_redraw` on change; registered as `art->cursor` on the window region ARegionType. `draw_pipeline_scroll()` now takes `hover_x`/`hover_y`; shared `compute_button_layout()` helper (eliminates duplicated geometry between draw and hit-test); selects fill from `COL_CARD`/`COL_CARD_HOVER`(`#323232`)/`COL_CARD_ACTIVE`/`COL_CARD_ACTIVE_HOVER` depending on hit + hover; calls `draw_rect_rounded` + `draw_rect_rounded_border` (`COL_ACCENT` = `#E87D0D` Blender orange placeholder) on hover. (4) **New color constants** — `COL_CARD_HOVER[4] = {0x32, 0x32, 0x32, 1.0}`, `COL_CARD_ACTIVE_HOVER[4] = {0x3E, 0x3E, 0x3E, 1.0}`, `COL_ACCENT[4] = {0xE8, 0x7D, 0x0D, 1.0}` (swappable once logo render lands). Pending human review: logo illustration, final accent hex, app icon, splash visual. Typeface: Inter (bundled) — permanent, no embed work needed.

**Claude AI contributor (2026-05-17, continued):** 0.7.0 Phase 2 CI fixes — Codex review bugs + MSVC compile errors + Scar 1 reorder operators. Branch `claude/phase-2-ai-implementation-nd4ZZ`. Commits `21ba8487` + `e496b6cd` (PR #198: alignment + `rna_enum_space_type_items` + MSVC fixes), `fd50eb93` (PR #199: Scar 1 reorder operators), `06d947d9` (PR #199: Codex fix — wrong return value on stubs). Key fixes: (1) **`SpaceBlendedLauncher` 64-bit alignment** — `mouse_x`/`mouse_y` replacing `_pad1[4]` made struct 52 bytes; restored `_pad1[4]` after the two int fields → 56 bytes. (2) **`SPACE_BLENDED_LAUNCHER` in `rna_enum_space_type_items`** (Codex Bug 1) — missing entry caused `LAUNCHER_HT_header` Python class to silently fail registration at startup. (3) **MSVC compile errors in `space_blended_launcher.cc`** — `LISTBASE_FOREACH` → range-based for; `MIN2`/`MAX2` → `std::min`/`std::max`; `static_cast<SpaceBlendedLauncher*>` → `reinterpret_cast`; `win->eventstate` → `win->runtime->eventstate` (stale hover fix, Codex Bug 2). (4) **Scar 1 workspace reorder operators** — `workspace_reorder_to_back/front_exec` in `workspace_edit.cc` called `BLI_listbase_rotate_last/first(&bmain->workspaces, ...)` which was removed with ID_WS in 0.2.0; surfaced as C2039 at CI step 6389/8095; initially stubbed to `OPERATOR_CANCELLED` — wrong. (5) **Codex catch: `OPERATOR_CANCELLED` vs `OPERATOR_FINISHED` on stubs** — `OPERATOR_CANCELLED` aborts macro/chain execution; a stub that is supposed to be a benign no-op must return `OPERATOR_FINISHED`. Also removed `workspace_context_poll` from both operators so they are disabled in the UI rather than appearing available and hard-failing. General rule: stub operators return `OPERATOR_FINISHED` with no poll.

**Claude AI contributor (2026-05-29, runtime audit):** 0.7.0 startup crash fix — `BKE_palette_add` namemap crash on deregistered ID_PAL. Branch `claude/0-7-0-phase2-materials-9WEBB`. Commit `25f59735`. PR #214. **Crash:** every launch of build 97 hit `EXCEPTION_ACCESS_VIOLATION` at `0xFFFFFFFFFFFFFFF8` before the window opened. Call chain: `WM_init` → `BLO_update_defaults_startup_blend` → `BKE_gpencil_palette_ensure` → `BKE_palette_add` → `BKE_id_new_name_validate` → `BKE_main_namemap_get_unique_name` → `namemap_get_name` → `Map<string>` array[-1] dereference → crash in string slot destructor. Root cause: the Scar 10 allocator wrote for `BKE_palette_add` called `BKE_id_new_name_validate`, which indexes the namemap by `BKE_idtype_idcode_to_index(ID_PAL)`. That returns -1 for the deregistered Scar 2 type, causing an out-of-bounds read at `maps[-1]` (8-byte pointer array). **Fix:** replaced `BKE_id_new_name_validate` with direct `BLI_strncpy_utf8(palette->id.name + 2, name, sizeof(palette->id.name) - 2)`. Palette is an internal type embedded in Brush — not user-visible named project data — so namemap uniqueness is not required. **General rule added to CLAUDE.md Category D:** never call `BKE_id_new_name_validate` for a deregistered Scar 2 type in a Scar 10 allocator; assign the name directly. Documented in Known Runtime Artifacts Category D. Caught at first 1.0.0-dev runtime audit session (build 97 was the first Blended release artifact ever run).
...
### ID_MSK — Mask ✓ complete (0.5.0 fold-down + 0.7.0 permanent home)
...
**0.7.0 permanent home (2026-05-17):** Commits `9c8dbc3c` (all layers) + `0d375e53` (Codex fixes). `NodeCompositeMask` storage added to `DNA_node_types.h` with node-owned `Mask*` + metadata fields `sfra/efra/flag/masklay_act/name[66]/_pad[6]`. `BKE_mask.hh` + `mask.cc`: `new_nodetree` / `copy_nodetree` / `free_nodetree` / `write_layers` / `read_layers` / `drain_from_bmain` helpers; `new_nodetree` sets `ID_MSK` prefix bytes so compositor cache keys are globally unique. `node_composite_mask.cc` rewritten with storage type + blend callbacks; write syncs metadata from mask into struct; read restores from struct into mask (efra==0 sentinel for pre-502.28 files); `template_id` removed. Versioning 502.27: compositor CMP_NODE_MASK nodes migrate mask from `node.id` into `NodeCompositeMask` storage (including metadata); sequencer strip and modifier mask references nullified (Category A). Drain called post-load in `readfile.cc`. Subversion 26 → 27 → 28. `bmain->masks` Category C leak resolved.
