...
> **(PR #157) `BLT_I18NCONTEXT_ID_CACHEFILE` borrowed by `NodesModifier`** — Two `NodesModifier` bake_target properties in `rna_modifier.cc` (lines 8027 and 8178) used `BLT_I18NCONTEXT_ID_CACHEFILE` as their `RNA_def_property_translation_context` argument — completely unrelated to CacheFile. The constant was removed from `BLT_translation.hh` in Layer 8, causing C2065 at step 5230/8093. Fix: removed the two `RNA_def_property_translation_context` calls (properties fall back to default context). **Scar 11 extension:** `BLT_I18NCONTEXT_ID_<TYPE>` constants can be borrowed by unrelated code across the codebase; they contain the type name but not the `ID_XX` token string. Mandatory post-chisel check added: `grep -rn "BLT_I18NCONTEXT_ID_<TYPE>" source/` after removing the constant from `BLT_translation.hh`.
