...
| 0.7.x | App lenses — launcher (§11), all 28 mode lenses (§12), full product identity (§16), `.blended` format design. Two phases: skeleton first, aesthetic second. | Phase 1 + Phase 2 partial ✓ CI-complete (build 97, commit `aa6ec698`). Logo ✓ + accent ✓ (`#2596be`). Pending: app icon, splash visual |
...
**Product identity:** Visual identity design complete. Full spec in BLENDED.md §16. Key decisions: logo = flat-vector orange blender appliance (`release/datafiles/blended_logo.svg` + `.png`). Accent = `#2596be` (final, definitive — replaces `#E87D0D` placeholder). Typeface = Inter (Blender's bundled interface font) for all surfaces — no custom embedding required.
