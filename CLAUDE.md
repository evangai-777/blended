...
**Launcher aesthetic:** Hybrid of Adobe Creative Cloud home screen (dark card surfaces, left sidebar primary actions, thumbnail file cards) and Blender splash (New File / Recent Files as startup concerns). §11 pipeline scroll is Blended's own. Three-level dark surface hierarchy: base `#1D1D1D` → panel `#252525` → card `#2C2C2C`. Interaction states are separate: card default `#2C2C2C`, hover `#323232`, active/pressed = accent color (Phase 2). Left sidebar ~220px: wordmark, [New Project] CTA, [Open…], recent file thumbnail cards (Adobe AssetCard pattern, ~160×120px, 8px corners), project settings gear, version attribution. Pipeline scroll in main area: "Blending?" at 28px SemiBold, section group separators at 11px all-caps letter-spaced, section headings at 16px SemiBold, mode buttons as rounded cards (8px corner radius, 16px/12px padding, hover lifts to `#323232`). Type stack: `"Source Sans Pro", -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif` — replaced by brand typeface in Phase 2. Spacing: 8px base unit. Hover animation: 130ms ease-out. Full spec in BLENDED.md §11 Launcher aesthetic.
