...
Six foundation layers — one minor version each, 1.0 when all six are honest. **0.6.x:** ✓ CI-complete (build 82) — depsgraph/draw/editor seam closure. **0.7.x:** ✓ CI-complete (build 99, commit `2ddd1dd0`) — launcher as canonical workspace system + all 28 mode lenses + full product identity (logo, accent `#ff7f00`, app icon, splash, typeface Inter) + `.blended` format design + all 6 Bucket 3 permanent homes + launcher header chrome + rounded mode button cards + hover state. Phase 2 design complete. **0.8.x:** ✓ CI-complete (build 100, commit `99e20b96`) — file format identity complete: magic bytes `"BLENDED"`, `.blended` extension on all write paths, one-way compat, `FILE_TYPE_BLENDED` bit flag, ~80 sites across ~22 source files + scripts + platform integration. **0.9.x:** in progress — `.blend` import: five-layer implementation plan complete (OB_MBALL versioning pass 502.31, `BKE_main_clear` Scar 2 drain, post-read drains for LS/PA/TE, `screen.cc` cleanup, dropped-data manifest). **1.0.0:** foundation complete, two concurrent workstreams — runtime audit (developer runs the build, checklists, triage loop with Claude) and GitHub Pages launch (landing page, marketing, tech demo). Release tag when both clear. Full roadmap in [`CHANGELOG.md`](../CHANGELOG.md).
